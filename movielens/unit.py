...
def ln_downstream(inputs, epsilon=1e-8, scope='ln_downstream'):
    with tf.variable_scope(scope, reuse=True):
        ...
        mean, variance = tf.nn.moments(inputs, [1], keep_dims=True)
        # tf.nn.moments calculate the mean and variance
        beta = tf.Variable(tf.truncated_normal(shape=[1], mean=0.0, stddev=0.02),trainable=True)

        gamma = tf.Variable(tf.truncated_normal(shape=[1], mean=0.0, stddev=0.02), trainable=True)

        # beta = tf.get_variable("beta_downstream", 1, initializer=tf.zeros_initializer())
        # gamma = tf.get_variable("gamma_downstream", 1, initializer=tf.ones_initializer())
        normalized = (inputs - mean) / ((variance + epsilon) ** (.5))
        # outputs = gamma * normalized + beta
        outputs = normalized
