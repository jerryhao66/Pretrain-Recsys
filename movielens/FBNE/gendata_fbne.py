...
class Dataset(object):
    ...
    def __init__(self, data_path):
        ...
        self.train_user_dict_2nd, self.valid_user_dict_2nd = self.generate_intra_users_2nd()
        self.train_user_dict_3rd, self.valid_user_dict_3rd = self.generate_intra_users_3rd()

        self.train_item_dict_2nd, self.valid_item_dict_2nd = self.generate_intra_items_2nd()
        self.train_item_dict_3rd, self.valid_item_dict_3rd = self.generate_intra_items_3rd()

    def generate_intra_users_2nd(self):
        train_user_dict, valid_user_dict = {}, {}
        with open('../movielens-upstream/FBNE_intra_file/movielens_2nd_user_train_file.csv', 'r') as f:
            line = f.readline()
        ...
        with open('../movielens-upstream/FBNE_intra_file/movielens_2nd_user_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
    ...
    def generate_intra_users_3rd(self):
        train_user_dict, valid_user_dict = {}, {}
        with open('../movielens-upstream/FBNE_intra_file/movielens_3rd_user_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                user, high_order_users = int(arr[0]), arr[1:]
        ...
        with open('../movielens-upstream/FBNE_intra_file/movielens_3rd_user_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                user, high_order_users = int(arr[0]), arr[1:]
    ...
    def generate_intra_items_2nd(self):
        ...
        with open('../movielens-upstream/FBNE_intra_file/movielens_2nd_item_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
        ...
        with open('../movielens-upstream/FBNE_intra_file/movielens_2nd_item_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
    ...
    def generate_intra_items_3rd(self):
        train_item_dict, valid_item_dict = {}, {}
        with open('../movielens-upstream/FBNE_intra_file/movielens_3rd_item_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
        ...
        with open('../movielens-upstream/FBNE_intra_file/movielens_3rd_item_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
    ...
    def get_positive_instances_user_task(self, random_seed, name):
        '''
        uesr-task
        '''
        np.random.seed(random_seed)
        batch_num = self.oracle_num_users // self.batch_size + 1
        target_user, k_shot_item, second_order_uesrs, oracle_user_ebd, mask_num_second_order_user, third_order_items, mask_num_third_order_item, intra_user_2nd, intra_user_3rd = [], [], [], [], [], [], [], [], []
        for batch in range(batch_num):
            b_target_u, b_k_shot_item, b_2nd_order_u, b_3rd_order_i, b_oracle_u_ebd, b_mask_num_2nd_u, b_mask_num_3rd_i, b_intra_u_2nd, b_intra_u_3rd = self._get_positive_batch_user_task(
                batch, name)
            ...
            intra_user_2nd.append(b_intra_u_2nd)
            intra_user_3rd.append(b_intra_u_3rd)

        return target_user, k_shot_item, second_order_uesrs, third_order_items, oracle_user_ebd, mask_num_second_order_user, mask_num_third_order_item, intra_user_2nd, intra_user_3rd

    def _get_positive_batch_user_task(self, i, name):
        batch_target_user, batch_kshot_item, batch_2nd_user, batch_oracle_user_ebd, mask_num_2nd_user, batch_3rd_item, mask_num_3rd_item, batch_intra_user_2nd, batch_intra_user_3rd = [], [], [], [], [], [], [], [], []
        ...
        for per_user_index in range(begin_index, end_index):
            ...
            if name == 'train':
                batch_intra_user_2nd.append(self.train_user_dict_2nd[target_user_id])
                batch_intra_user_3rd.append(self.train_user_dict_3rd[target_user_id])
            else:
                batch_intra_user_2nd.append(self.valid_user_dict_2nd[target_user_id])
                batch_intra_user_3rd.append(self.valid_user_dict_3rd[target_user_id])
        ...
        return batch_target_user, batch_kshot_item, batch_2nd_user_input, batch_3rd_item_input, batch_oracle_user_ebd, mask_num_2nd_user, mask_num_3rd_item, batch_intra_user_2nd, batch_intra_user_3rd

    '''
    mix item-task
    '''

    def get_positive_instances_item_task(self, random_seed, name):
        np.random.seed(random_seed)
        batch_num = self.oracle_num_items // self.batch_size + 1
        target_item, k_shot_user, second_order_items, oracle_item_ebd, mask_num_second_order_item, third_order_users, mask_num_third_order_user, intra_item_2nd, intra_item_3rd = [], [], [], [], [], [], [], [], []
        for batch in range(batch_num):
            b_target_i, b_k_shot_user, b_2nd_order_i, b_3rd_order_u, b_oracle_i_ebd, b_mask_num_2nd_i, b_mask_num_3rd_u, b_intra_i_2nd, b_intra_i_3rd = self._get_positive_batch_item_task(
                batch, name)
            ...
            intra_item_2nd.append(b_intra_i_2nd)
            intra_item_3rd.append(b_intra_i_3rd)

        return target_item, k_shot_user, second_order_items, third_order_users, oracle_item_ebd, mask_num_second_order_item, mask_num_third_order_user, intra_item_2nd, intra_item_3rd

    def _get_positive_batch_item_task(self, i, name):
        batch_target_item, batch_kshot_user, batch_2nd_item, batch_oracle_item_ebd, mask_num_2nd_item, batch_3rd_user, mask_num_3rd_user, batch_intra_item_2nd, batch_intra_item_3rd = [], [], [], [], [], [], [], [], []
        ...
        for per_item_index in range(begin_index, end_index):
            ...
            if name =='train':
                batch_intra_item_2nd.append(self.train_item_dict_2nd[target_item_id])
                batch_intra_item_3rd.append(self.train_item_dict_3rd[target_item_id])
            else:
                batch_intra_item_2nd.append(self.valid_item_dict_2nd[target_item_id])
                batch_intra_item_3rd.append(self.valid_item_dict_3rd[target_item_id])
        ...
        return batch_target_item, batch_kshot_user, batch_2nd_item_input, batch_3rd_user_input, batch_oracle_item_ebd, mask_num_2nd_item, mask_num_3rd_user, batch_intra_item_2nd, batch_intra_item_3rd
