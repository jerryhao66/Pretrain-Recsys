...
from unit import *
import gendata_fbne as fbne_data
...
def evaluate_user_task(valid_batch_index, model, sess, valid_data, is_training):
    ...
    for index in tqdm.tqdm(valid_batch_index):
        user_id, support_item, target_user = data.batch_gen_user_task(valid_data, index, setting.batch_size_user)
...
def training_batch_user_task(batch_index, model, sess, train_data, is_training):
    for index in batch_index:
        _, support_item, target_user = data.batch_gen_user_task(train_data, index, setting.batch_size_user)
...
def training_loss_user_task(batch_index, model, sess, train_data, is_training):
    train_loss = 0.0
    num_batch = int(train_data[3] / setting.batch_size_user)
    for index in batch_index:
        _, support_item, target_user = data.batch_gen_user_task(train_data, index, setting.batch_size_user)
        feed_dict = {model.support_item: support_item,
                     model.target_user: target_user,
...
def training_batch_item_task(batch_index, model, sess, train_data, is_training):
    for index in batch_index:
        _, support_user, target_item = data.batch_gen_item_task(train_data, index, setting.batch_size_item)
...
def training_loss_item_task(batch_index, model, sess, train_data, is_training):
    train_loss = 0.0
    num_batch = int(train_data[3] / setting.batch_size_item)
    for index in batch_index:
        _, support_user, target_item = data.batch_gen_item_task(train_data, index, setting.batch_size_item)
        feed_dict = {model.support_user: support_user,
                     model.target_item: target_item,
...
def training_item_task(model, sess):
    ...
    num_batch = int(train_batches[3]) // setting.batch_size_item
    batch_index = range(num_batch)
    valid_num_batch = int(valid_batches[3]) // setting.batch_size_item
    valid_batch_index = range(valid_num_batch)
...
def evaluate_item_task(valid_batch_index, model, sess, valid_data, is_training):
    ...
    evaluate_loss = 0.0
    for index in tqdm.tqdm(valid_batch_index):
        item_id, support_user, target_item = data.batch_gen_item_task(valid_data, index, setting.batch_size_item)
...
def training_2nd_item_task(model, sess):
    ...
    num_batch_train = data_train.oracle_num_items // setting.batch_size_item + 1
    train_batch_index = range(num_batch_train)
    ...
    num_batch_valid = data_valid.oracle_num_items // setting.batch_size_item + 1
    valid_batch_index = range(num_batch_valid)
...
def training_loss_2nd_item_task(data, batch_index, model, sess, train_data, is_training):
    train_loss = 0.0
    num_batch = data.oracle_num_items // setting.batch_size_item
    for index in batch_index:
        b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user = data.batch_gen_3rd_item_task(
            train_data, index)
        feed_dict = {model.target_item: b_oracle_item_ebd, model.support_user_1st_pos: b_k_shot_user,
...
def training_user_task_fbne(model, sess):
    best_loss = 0
    saver = tf.train.Saver()
    ts_u = fbne_data.generate_user_dict_train(setting.oracle_training_file_user_task)

    vs_u = fbne_data.generate_user_dict_valid(setting.oracle_valid_file_user_task)
    train_batches = fbne_data.generate_meta_train_user_set(ts_u)
    valid_batches = fbne_data.generate_meta_valid_user_set(vs_u)
    ...
    for epoch_count in range(setting.user_epoch):
        ...
        if epoch_count % setting.verbose == 0:
            loss_begin = time()
            train_loss = training_loss_user_task_fbne(batch_index, model, sess, train_batches, True)
            loss_time = time() - loss_begin
            eval_begin = time()
            cosine = evaluate_user_task_fbne(valid_batch_index, model, sess, valid_batches, True)
            eval_time = time() - eval_begin
            print(
                'epoch %d, train time is %.4f, loss time is %.4f, eval_time is %.4f, train_loss is %.4f, test cosine value is %.4f' % (
                    epoch_count, train_time, loss_time, eval_time, train_loss, cosine))
...
def evaluate_user_task_fbne(valid_batch_index, model, sess, valid_data, is_training):
...
def training_batch_user_task_fbne(batch_index, model, sess, train_data, is_training):
    for index in batch_index:
        _, support_item, target_user = data.batch_gen_user_task(train_data, index, setting.batch_size_user)
        feed_dict = {model.support_item: support_item,
                     model.target_user: target_user,
...
def training_loss_user_task_fbne(batch_index, model, sess, train_data, is_training):
    train_loss = 0.0
    num_batch = int(train_data[3] / setting.batch_size_user)
    for index in batch_index:
        _, support_item, target_user = fbne_data.batch_gen_user_task(train_data, index, setting.batch_size_user)
        feed_dict = {model.support_item: support_item,
                     model.target_user: target_user,
                     model.training_phrase_user_task: is_training}
        train_loss += sess.run(model.loss_user_task, feed_dict)
...
def training_batch_item_task_fbne(batch_index, model, sess, train_data, is_training):
    for index in batch_index:
        _, support_user, target_item = fbne_data.batch_gen_item_task(train_data, index, setting.batch_size_item)
        feed_dict = {model.support_user: support_user,
                     model.target_item: target_item,
                     model.training_phrase_item_task: is_training}
        sess.run([model.loss_item_task, model.optimizer_item_task], feed_dict)
...
def training_loss_item_task_fbne(batch_index, model, sess, train_data, is_training):
    train_loss = 0.0
    num_batch = int(train_data[3] / setting.batch_size_item)
    for index in batch_index:
        _, support_user, target_item = fbne_data.batch_gen_item_task(train_data, index, setting.batch_size_item)
...
def training_item_task_fbne(model, sess):
    best_loss = 0
    saver = tf.train.Saver(max_to_keep=3)
    ts_i = fbne_data.generate_item_dict_train(setting.oracle_training_file_item_task)
    vs_i = fbne_data.generate_item_dict_valid(setting.oracle_valid_file_item_task)

    train_batches = fbne_data.generate_meta_train_item_set(ts_i)
    valid_batches = fbne_data.generate_meta_valid_item_set(vs_i)
    ...
    for epoch_count in range(setting.user_epoch):
        train_begin = time()
        training_batch_item_task_fbne(batch_index, model, sess, train_batches, True)
        train_time = time() - train_begin
        if epoch_count % setting.verbose == 0:
            loss_begin = time()
            train_loss = training_loss_item_task_fbne(batch_index, model, sess, train_batches, True)
            loss_time = time() - loss_begin
            eval_begin = time()
            cosine = evaluate_item_task_fbne(valid_batch_index, model, sess, valid_batches, True)
            eval_time = time() - eval_begin
            print(
                'epoch %d, train time is %.4f, loss time is %.4f, eval_time is %.4f, train_loss is %.4f, test cosine value is %.4f' % (
                    epoch_count, train_time, loss_time, eval_time, train_loss, cosine))
...
def evaluate_item_task_fbne(valid_batch_index, model, sess, valid_data, is_training):
    
    evaluate_loss = 0.0
    for index in tqdm.tqdm(valid_batch_index):
        item_id, support_user, target_item = fbne_data.batch_gen_item_task(valid_data, index, setting.batch_size_item)
...
def training_batch_2nd_user_task_fbne(fbne_data, batch_index, model, sess, train_data, is_training):
    for index in batch_index:
        b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item, b_intra_2nd_user, b_intra_3rd_user = fbne_data.batch_gen_3rd_user_task(
            train_data, index)
       
        feed_dict = {model.target_user: b_oracle_user_ebd, model.support_item_1st: b_k_shot_item,
                     model.training_phrase_user_task: is_training, model.support_user_2nd: b_second_order_users,
                     model.inter_support_2nd_user: b_intra_2nd_user,
                     model.training_phrase_item_task: is_training}

        sess.run([model.loss_2nd_user, model.optimizer_2nd_user_task], feed_dict)


def training_loss_2nd_user_task_fbne(fbne_data, batch_index, model, sess, train_data, is_training):
    train_loss = 0.0
    num_batch = fbne_data.oracle_num_users // setting.batch_size_user
    for index in batch_index:
        b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item, b_intra_2nd_user, b_intra_3rd_user = fbne_data.batch_gen_3rd_user_task(
            train_data, index)
        feed_dict = {model.target_user: b_oracle_user_ebd, model.support_item_1st: b_k_shot_item,
...
def evaluate_2nd_user_task_fbne(fbne_data, valid_batch_index, model, sess, valid_data, is_training):
    evaluate_loss, evaluate_pearson = 0.0, 0.0
    for index in tqdm.tqdm(valid_batch_index):
        b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item, b_intra_2nd_user, b_intra_3rd_user = fbne_data.batch_gen_3rd_user_task(
            valid_data, index)
...
def training_3rd_user_task_fbne(model, sess):
    best_loss = 0
    saver = tf.train.Saver()
    ...
    for epoch_count in range(setting.third_user_epoch):
        train_begin = time()
        training_batch_3rd_user_task_fbne(data_train, train_batch_index, model, sess, train_batches, True)
        train_time = time() - train_begin
        if epoch_count % setting.verbose == 0:
            loss_begin = time()
            train_loss = training_loss_3rd_user_task_fbne(data_train, train_batch_index, model, sess, train_batches, True)
            loss_time = time() - loss_begin
            eval_begin = time()
            cosine, pearson = evaluate_3rd_user_task_fbne(data_valid, valid_batch_index, model, sess, valid_batches, False)
            eval_time = time() - eval_begin
            print(
                'epoch %d, train time is %.4f, loss time is %.4f, eval_time is %.4f, train_loss is %.4f, test cosine value is %.4f, test pearson value is %.4f' % (
                    epoch_count, train_time, loss_time, eval_time, train_loss, cosine, pearson))
...
def training_batch_3rd_user_task_fbne(fbne_data, batch_index, model, sess, train_data, is_training):
    for index in batch_index:
        b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item, b_intra_2nd_user, b_intra_3rd_user = fbne_data.batch_gen_3rd_user_task(
            train_data, index)
        feed_dict = {model.target_user: b_oracle_user_ebd, model.support_item_1st_: b_k_shot_item,
                     model.training_phrase_user_task: is_training, model.support_user_2nd_: b_second_order_users,
                     model.training_phrase_item_task: is_training,
                     model.inter_support_3rd_user: b_intra_3rd_user,
                     model.support_item_3rd: b_third_order_items}

        sess.run([model.loss_3rd_user, model.optimizer_3rd_user_task], feed_dict)


def training_loss_3rd_user_task_fbne(fbne_data, batch_index, model, sess, train_data, is_training):
    train_loss = 0.0
    num_batch = fbne_data.oracle_num_users // setting.batch_size_user
    for index in batch_index:
        b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item, b_intra_2nd_user, b_intra_3rd_user = fbne_data.batch_gen_3rd_user_task(
...
def training_2nd_item_task_fbne(model, sess):
    ...
    data_valid = fbne_data.Dataset(setting.oracle_valid_file_item_task)
    ...
    for epoch_count in range(setting.second_item_epoch):
        train_begin = time()
        training_batch_2nd_item_task_fbne(data_train, train_batch_index, model, sess, train_batches, True)
        train_time = time() - train_begin
        if epoch_count % setting.verbose == 0:
            loss_begin = time()
            train_loss = training_loss_2nd_item_task_fbne(data_train, train_batch_index, model, sess, train_batches, True)
            loss_time = time() - loss_begin
            eval_begin = time()
            cosine, pearson = evaluate_2nd_item_task_fbne(data_valid, valid_batch_index, model, sess, valid_batches, False)
            eval_time = time() - eval_begin
            print(
                'epoch %d, train time is %.4f, loss time is %.4f, eval_time is %.4f, train_loss is %.4f, test cosine value is %.4f, test pearson value is %.4f' % (
                    epoch_count, train_time, loss_time, eval_time, train_loss, cosine, pearson))
        ...
        data_train = fbne_data.Dataset(setting.oracle_training_file_item_task)
        train_batches = data_train.get_positive_instances_item_task(epoch_count + 1, 'train')
        num_batch_train = data_train.oracle_num_items // setting.batch_size_item + 1
        train_batch_index = range(num_batch_train)
...
def training_batch_2nd_item_task_fbne(fbne_data, batch_index, model, sess, train_data, is_training):
    for index in batch_index:
        b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user, b_intra_2nd_item, b_intra_3rd_item = fbne_data.batch_gen_3rd_item_task(
            train_data, index)
       
        feed_dict = {model.target_item: b_oracle_item_ebd, model.support_user_1st_pos: b_k_shot_user,
                     model.training_phrase_user_task: is_training, model.support_item_2nd_pos: b_second_order_items,
                     model.inter_support_2nd_item_pos: b_intra_2nd_item,
                     model.training_phrase_item_task: is_training}

        sess.run([model.loss_2nd_item_pos, model.optimizer_2nd_item_task_pos], feed_dict)


def training_loss_2nd_item_task_fbne(fbne_data, batch_index, model, sess, train_data, is_training):
    train_loss = 0.0
    num_batch = fbne_data.oracle_num_items // setting.batch_size_item
    for index in batch_index:
        b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user, b_intra_2nd_item, b_intra_3rd_item  = fbne_data.batch_gen_3rd_item_task(
            train_data, index)
        feed_dict = {model.target_item: b_oracle_item_ebd, model.support_user_1st_pos: b_k_shot_user,
...
def evaluate_2nd_item_task_fbne(fbne_data, valid_batch_index, model, sess, valid_data, is_training):
    evaluate_loss, evaluate_pearson = 0.0, 0.0
    for index in tqdm.tqdm(valid_batch_index):
        b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user, b_intra_2nd_item, b_intra_3rd_item = fbne_data.batch_gen_3rd_item_task(
            valid_data, index)
...
def training_3rd_item_task_fbne(model, sess):
    best_loss = 0
    saver = tf.train.Saver()
    ...
    for epoch_count in range(setting.third_item_epoch):
        train_begin = time()
        training_batch_3rd_item_task_fbne(data_train, train_batch_index, model, sess, train_batches, True)
        train_time = time() - train_begin
        if epoch_count % setting.verbose == 0:
            loss_begin = time()
            train_loss = training_loss_3rd_item_task_fbne(data_train, train_batch_index, model, sess, train_batches, True)
            loss_time = time() - loss_begin
            eval_begin = time()
            cosine, pearson = evaluate_3rd_item_task_fbne(data_valid, valid_batch_index, model, sess, valid_batches, False)
            eval_time = time() - eval_begin
            print(
                'epoch %d, train time is %.4f, loss time is %.4f, eval_time is %.4f, train_loss is %.4f, test cosine value is %.4f, test pearson value is %.4f' % (
                    epoch_count, train_time, loss_time, eval_time, train_loss, cosine, pearson))
...
def training_batch_3rd_item_task_fbne(fbne_data, batch_index, model, sess, train_data, is_training):
    for index in batch_index:
        b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user, b_intra_2nd_item, b_intra_3rd_item = fbne_data.batch_gen_3rd_item_task(
            train_data, index)
        feed_dict = {model.target_item: b_oracle_item_ebd, model.support_user_1st_pos_: b_k_shot_user,
                     model.training_phrase_user_task: is_training, model.support_item_2nd_pos_: b_second_order_items,
                     model.training_phrase_item_task: is_training,
                     model.inter_support_3rd_item_pos: b_intra_3rd_item,
                     model.support_user_3rd_pos: b_third_order_users}
...
def evaluate_3rd_item_task_fbne(fbne_data, valid_batch_index, model, sess, valid_data, is_training):
    evaluate_loss, evaluate_pearson = 0.0, 0.0
    for index in tqdm.tqdm(valid_batch_index):
        b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user, b_intra_2nd_item, b_intra_3rd_item = fbne_data.batch_gen_3rd_item_task(
            valid_data, index)
