...
def train_user_rl_task(name):
    ...
    batch_size_user = setting.batch_size_user
            ...
            for epoch in range(40):
                ...
                for rl_epoch in range(1):
                    ...
                    for batch_index in tqdm.tqdm(range(num_batch_train)):
                        ...
                        sampled_rewards = np.zeros((sample_times, batch_size_user), dtype=np.float32)
                        sampled_states = np.zeros((sample_times, batch_size_user, max_num, state_size), dtype=np.float32)
                        sampled_actions = np.zeros((sample_times, batch_size_user, max_num), dtype=np.float32)

                        sampled_rewards_3rd = np.zeros((sample_times, batch_size_user), dtype=np.float32)
                        sampled_states_3rd = np.zeros((sample_times, batch_size_user, max_num_3rd, state_size_3rd),
                                                      dtype=np.float32)
                        sampled_actions_3rd = np.zeros((sample_times, batch_size_user, max_num_3rd), dtype=np.float32)

                        avg_reward = np.zeros((batch_size_user), dtype=np.float32)
...
def train_item_rl_task(name):
    env = environment()
    sample_times = setting.sample_times
    batch_size_item = setting.batch_size_item
            ...
            for epoch in range(40):
                ...
                for rl_epoch in range(1):
                    ...
                    for batch_index in tqdm.tqdm(range(num_batch_train)):
                        ...
                        sampled_rewards = np.zeros((sample_times, batch_size_item), dtype=np.float32)
                        sampled_states = np.zeros((sample_times, batch_size_item, max_num, state_size), dtype=np.float32)
                        sampled_actions = np.zeros((sample_times, batch_size_item, max_num), dtype=np.float32)

                        sampled_rewards_3rd = np.zeros((sample_times, batch_size_item), dtype=np.float32)
                        sampled_states_3rd = np.zeros((sample_times, batch_size_item, max_num_3rd, state_size_3rd),
                                                      dtype=np.float32)
                        sampled_actions_3rd = np.zeros((sample_times, batch_size_item, max_num_3rd), dtype=np.float32)

                        avg_reward = np.zeros((batch_size_item), dtype=np.float32)
                        for sample_time in range(sample_times):
                            # 2rd
