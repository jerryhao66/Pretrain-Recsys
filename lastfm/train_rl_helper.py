...
class environment():
    ...
    def parse_train_batches(self, train_batches, batch_index):
        return [(train_batches[r])[batch_index] for r in range(7)]
    ...
    def initilize_state_user_task(self, max_num, state_size, batch_original_reward, original_reward):
        ...
        self.origin_prob = np.zeros((self.batch_size_user, 1), dtype=np.float32) 

        self.dot_product = np.zeros((self.batch_size_user, 1), dtype=np.float32) 

        self.element_wise_current_mean1 = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum1 = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean2 = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum2 = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean3 = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum3 = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)

        self.vector_current_sum1 = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)

        self.vector_current_mean1 = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)

        self.num_selected = np.zeros(self.batch_size_user, dtype=np.int)
        self.action_matrix = np.zeros((self.batch_size_user, self.max_num), dtype=np.int)
        self.state_matrix = np.zeros((self.batch_size_user, self.max_num, state_size), dtype=np.float32)
        self.selected_input = np.full((self.batch_size_user, self.max_num), self.padding_number_user_task)

    def initilize_state_user_task_3rd(self, max_num_3rd, state_size_3rd):
        self.max_num_3rd = max_num_3rd
        self.origin_prob_3rd = np.zeros((self.batch_size_user, 1), dtype=np.float32)  
        self.dot_product_3rd = np.zeros((self.batch_size_user, 1), dtype=np.float32) 
        self.element_wise_current_mean1_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum1_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean2_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum2_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean3_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum3_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean4_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum4_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean5_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum5_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.vector_current_sum1_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.vector_current_mean1_3rd = np.zeros((self.batch_size_user, self.embedding_size), dtype=np.float32)
        self.num_selected_3rd = np.zeros(self.batch_size_user, dtype=np.int)
        self.action_matrix_3rd = np.zeros((self.batch_size_user, self.max_num_3rd), dtype=np.int)
        self.state_matrix_3rd = np.zeros((self.batch_size_user, self.max_num_3rd, state_size_3rd), dtype=np.float32)
        self.selected_input_3rd = np.full((self.batch_size_user, self.max_num_3rd), self.padding_number_user_task)
    ...
    def initilize_state_item_task(self, max_num, state_size, batch_original_reward, original_reward):
        ...
        self.batch_original_reward = batch_original_reward
        self.original_reward = original_reward
        self.origin_prob = np.zeros((self.batch_size_item, 1), dtype=np.float32)  
        self.dot_product = np.zeros((self.batch_size_item, 1), dtype=np.float32)  
        self.element_wise_current_mean1 = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum1 = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean2 = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum2 = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean3 = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum3 = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.vector_current_sum1 = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.vector_current_mean1 = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.num_selected = np.zeros(self.batch_size_item, dtype=np.int)
        self.action_matrix = np.zeros((self.batch_size_item, self.max_num), dtype=np.int)
        self.state_matrix = np.zeros((self.batch_size_item, self.max_num, state_size), dtype=np.float32)
        self.selected_input = np.full((self.batch_size_item, self.max_num), self.padding_number_item_task)

    def initilize_state_item_task_3rd(self, max_num_3rd, state_size_3rd):
        self.max_num_3rd = max_num_3rd
        self.element_wise_current_mean1_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum1_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean2_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum2_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean3_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum3_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean4_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum4_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean5_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum5_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.vector_current_sum1_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.vector_current_mean1_3rd = np.zeros((self.batch_size_item, self.embedding_size), dtype=np.float32)
        self.num_selected_3rd = np.zeros(self.batch_size_item, dtype=np.int)
        self.action_matrix_3rd = np.zeros((self.batch_size_item, self.max_num_3rd), dtype=np.int)
        self.state_matrix_3rd = np.zeros((self.batch_size_item, self.max_num_3rd, state_size_3rd), dtype=np.float32)
        self.selected_input_3rd = np.full((self.batch_size_item, self.max_num_3rd), self.padding_number_item_task)
    ...
    def get_selected_second_order_neighbors_item_task(self, b_second_order_item):
        ...
        row_deleted_flag = np.ones((self.batch_size_item), dtype=np.int)

        selected_input = np.zeros((self.batch_size_item, self.max_num), dtype=np.int)
        for batch_index in range(self.batch_size_item):
            selected = []
    ...
    def get_selected_third_order_neighbors_item_task(self, b_third_order_users, row_deleted_flag_2rd):
        row_deleted_index = []
        selected_input = np.zeros((self.batch_size_item, self.max_num_3rd), dtype=np.int)
        for batch_index in range(self.batch_size_item):
            selected = []
            ...
            self.num_selected_3rd = self.batch_size_item * self.max_num_3rd - np.sum(
    ...
    def get_selected_second_order_neighbors_user_task(self, b_second_order_users):
        ...
        row_deleted_flag = np.ones((self.batch_size_user), dtype=np.int)

        selected_input = np.zeros((self.batch_size_user, self.max_num), dtype=np.int)
        for batch_index in range(self.batch_size_user):
            selected = []
            for neighbor_index in range(self.max_num):
                if self.action_matrix[batch_index, neighbor_index] == 1:
    ...
    def get_selected_third_order_neighbors_user_task(self, b_third_order_items, row_deleted_flag_2rd):
        row_deleted_index = []
        selected_input = np.zeros((self.batch_size_user, self.max_num_3rd), dtype=np.int)
        for batch_index in range(self.batch_size_user):
            selected = []
            for neighbor_index in range(self.max_num_3rd):
                if self.action_matrix_3rd[batch_index, neighbor_index] == 1:
            ...
            self.num_selected_3rd = self.batch_size_user * self.max_num_3rd - np.sum(
...
def generate_batch_user_task(batch_index, model, sess, train_data, is_training, num_instances):
    ...
    for index in batch_index:
        _, support_item = data.batch_gen_task(train_data, index, setting.batch_size_user, num_instances)
        ...
        start_index = index * setting.batch_size_user
        end_index = min(start_index + setting.batch_size_user, num_instances)
...
def generate_batch_item_task(batch_index, model, sess, train_data, is_training, num_instances):
    ...
    for index in batch_index:
        all_item_id, support_user = data.batch_gen_task(train_data, index, setting.batch_size_item, num_instances)
...
def train_user_rl_task(name):
    ...
    batch_size = setting.batch_size_user
    ...
    with g_rl.as_default():
        with sess2.as_default():
            ...
            for epoch in range(40): 
                train_batches = ClassData.get_positive_instances_user_task(epoch)
                num_batch_train = ClassData.oracle_num_users // setting.batch_size_user

                all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3 = model.generate_batch_state_ebd_user_task(
                ...
                for rl_epoch in range(1):
                    ...
                    for batch_index in tqdm.tqdm(range(num_batch_train)):
                        ...
                        b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item = env.parse_train_batches(
                        ...
                        original_reward_2rd = model.produce_original_reward_user_task(train_batches, num_batch_train,
                                                                                      ClassData)
                        original_reward_3rd = model.produce_original_reward_3rd_user_task(train_batches,
                                                                                          num_batch_train,
                                                                                          ClassData)
                        ...
                        for step_index in range(max_num_3rd):
                            ...
                            # print(b_second_order_users_array.shape)
...
def revise_rl_user_task(name):
    ...
    with g_rl_revise.as_default():
        with sess3.as_default():
            ...
            num_batch_valid = data_valid.oracle_num_users // setting.batch_size_user
            ...
            for batch_index in tqdm.tqdm(valid_batch_index):
                ...
                original_reward_2rd = model.produce_original_reward_user_task(valid_batches, num_batch_valid,
                                                                              data_valid)
                original_reward_3rd = model.produce_original_reward_3rd_user_task(valid_batches, num_batch_valid,
                                                                                  data_valid)
                ...
                reward, pearson = model.produce_reward_3rd_user_task(b_target_user, b_k_shot_item, selected_input_2rd,
                                                                     selected_input_3rd,
                                                                     b_oracle_user_ebd)
                # if selected neighbors is none in one row, we put the original reward to this situation
                for del_idx in row_deleted_index_3rd:
...
def train_item_rl_task(name):
    env = environment()
    sample_times = setting.sample_times
    batch_size = setting.batch_size_item
    ...
    with g_rl.as_default():
        with sess2.as_default():
            ...
            for epoch in range(40):
                train_batches = ClassData.get_positive_instances_item_task(epoch)
                num_batch_train = ClassData.oracle_num_items // setting.batch_size_item

                all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3 = model.generate_batch_state_ebd_item_task(
                    train_batches, ClassData)
                ...
                for rl_epoch in range(1):
                    ...
                    for batch_index in tqdm.tqdm(range(num_batch_train)):
                        ...
                        b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user = env.parse_train_batches(
                            train_batches, batch_index)

                        b_target_ori_ebd, b_target_aggre_ebd, b_target_second_aggre_ebd, b_first_neigh_ori_ebd, b_second_neigh_ori_ebd, batch_third_neigh_ori_ebd, b_prob1, b_prob2, b_prob3 = \
                            all_target_ori_ebd[
                                batch_index], \
                            all_target_aggre_ebd[
                                batch_index], \
                        ...
                        original_reward_2rd = model.produce_original_reward_item_task(train_batches, num_batch_train,
                                                                                      ClassData)
                        original_reward_3rd = model.produce_original_reward_3rd_item_task(train_batches,
                                                                                          num_batch_train,
                                                                                          ClassData)
...
def revise_rl_item_task(name):
    ...
    with g_rl_revise.as_default():
        with sess3.as_default():
            ...
            num_batch_valid = data_valid.oracle_num_items // setting.batch_size_item
            valid_batch_index = range(num_batch_valid)
            ...
            for batch_index in tqdm.tqdm(valid_batch_index):
                ...
                b_target_ori_ebd, b_target_aggre_ebd, b_target_second_aggre_ebd, b_first_neigh_ori_ebd, b_second_neigh_ori_ebd, b_third_neigh_ori_ebd, b_prob1, b_prob2, b_prob3 = \
                    ...
                    all_first_neigh_ori_ebd[
                        batch_index], \
                    all_second_neigh_ori_ebd[
                        batch_index], all_third_neigh_ori_ebd[batch_index], all_first_order_reward1[batch_index], \
                    all_first_and_second_order_reward2[batch_index], all_former_third_order_reward3[batch_index]

                max_num = b_second_neigh_ori_ebd.shape[1]
                max_num_3rd = b_third_neigh_ori_ebd.shape[1]

                original_reward_2rd = model.produce_original_reward_item_task(valid_batches, num_batch_valid,
                                                                              data_valid)
                original_reward_3rd = model.produce_original_reward_3rd_item_task(valid_batches, num_batch_valid,
                                                                                  data_valid)
