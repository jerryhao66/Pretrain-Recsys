...
class DataLoader(object):
    def __init__(self, path1, path2):
        ...
        self.padding_dict_user, self.padding_dict_item = self._generate_padding_dict()

        self.train_user_dict_2nd, self.test_user_dict_2nd = self.generate_intra_users_2nd()
        self.train_user_dict_3rd, self.test_user_dict_3rd = self.generate_intra_users_3rd()

        self.train_item_dict_2nd, self.test_item_dict_2nd = self.generate_intra_items_2nd()
        self.train_item_dict_3rd, self.test_item_dict_3rd = self.generate_intra_items_3rd()


    def _generate_padding_dict(self):
        padding_dict_user, padding_dict_item = {}, {}
        with open('../lastfm-upstream/user_task/user_task_train_oracle_rating.csv', 'r') as f:
            line = f.readline()
            while line!="" and line!=None:
                arr = line.strip().split(',')
                user, item = int(arr[0]), int(arr[1])
                if user not in padding_dict_user:
                    padding_dict_user[user] = []
                    padding_dict_user[user].append(item)
                else:
                    padding_dict_user[user].append(item)
    
                if item not in padding_dict_item:
                    padding_dict_item[item] = []
                    padding_dict_item[item].append(user)
                else:
                    padding_dict_item[item].append(user)
        ...
        with open('../lastfm-upstream/user_task/user_task_valid_oracle_rating.csv', 'r') as f:
            line = f.readline()
            while line!="" and line!=None:
                arr = line.strip().split(',')
                user, item = int(arr[0]), int(arr[1])
        ...
        with open('../lastfm_downstream/lastfm/user_task_meta_train.csv', 'r') as f:
            line = f.readline()
            while line!="" and line!=None:
                arr = line.strip().split(',')
                user, item = int(arr[0]), int(arr[1])
        ...
        return padding_dict_user, padding_dict_item


    def generate_intra_users_2nd(self):
        train_user_dict, valid_user_dict = {}, {}
        with open('./FBNE_intra_file/lastfm_2nd_user_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                user, high_order_users = int(arr[0]), arr[1:]
                train_user_dict[user] = []
                for high_order_user in high_order_users:
                    if high_order_user != "":
                        train_user_dict[user].append(int(high_order_user))

                line = f.readline()

        with open('./FBNE_intra_file/lastfm_2nd_user_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                user, high_order_users = int(arr[0]), arr[1:]
                ...
                for high_order_user in high_order_users:
                    if high_order_user != "":
                        valid_user_dict[user].append(int(high_order_user))

                line = f.readline()

        with open('./FBNE_intra_file_upstream/lastfm_2nd_user_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                user, high_order_users = int(arr[0]), arr[1:]
        ...
        with open('./FBNE_intra_file_upstream/lastfm_2nd_user_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                user, high_order_users = int(arr[0]), arr[1:]
        ...
        for user in range(setting.num_users):
            if user not in train_user_dict.keys():
                train_user_dict[user] = []
                for _ in range(6):# 8 is intra length
                    train_user_dict[user].append(0) 

        for user in range(setting.num_users):
            if user not in valid_user_dict.keys():
                valid_user_dict[user] = []
                for _ in range(6): # 8 is intra length
                    valid_user_dict[user].append(0) 


        return train_user_dict, valid_user_dict


    def generate_intra_users_3rd(self):
        train_user_dict, valid_user_dict = {}, {}
        with open('./FBNE_intra_file/lastfm_3rd_user_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                user, high_order_users = int(arr[0]), arr[1:]
        ...
        with open('./FBNE_intra_file/lastfm_3rd_user_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                user, high_order_users = int(arr[0]), arr[1:]
        ...
        with open('./FBNE_intra_file_upstream/lastfm_3rd_user_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                user, high_order_users = int(arr[0]), arr[1:]
        ...
        with open('./FBNE_intra_file_upstream/lastfm_3rd_user_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                user, high_order_users = int(arr[0]), arr[1:]
    ...
    def generate_intra_items_2nd(self):
        train_item_dict, valid_item_dict = {}, {}
        with open('./FBNE_intra_file/lastfm_2nd_item_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
                train_item_dict[item] = []
                for high_order_item in high_order_items:
                    if high_order_item != "":
                        train_item_dict[item].append(int(high_order_item))

                line = f.readline()

        with open('./FBNE_intra_file/lastfm_2nd_item_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
                ...
                for high_order_item in high_order_items:
                    if high_order_item != "":
                        valid_item_dict[item].append(int(high_order_item))

                line = f.readline()

        with open('./FBNE_intra_file_upstream/lastfm_2nd_item_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
        ...
        with open('./FBNE_intra_file_upstream/lastfm_2nd_item_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
        ...
        for item in range(setting.num_items):
            if item not in train_item_dict.keys():
                train_item_dict[item] = []
                for _ in range(6): # 8 is intra length
                    train_item_dict[item].append(0) 

        for item in range(setting.num_items):
            if item not in valid_item_dict.keys():
                valid_item_dict[item] = []
                for _ in range(6): # 8 is intra length 
                    valid_item_dict[item].append(0) 

        return train_item_dict, valid_item_dict

    def generate_intra_items_3rd(self):
        train_item_dict, valid_item_dict = {}, {}
        with open('./FBNE_intra_file/lastfm_3rd_item_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
        ...
        with open('./FBNE_intra_file/lastfm_3rd_item_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
        ...
        with open('./FBNE_intra_file_upstream/lastfm_3rd_item_train_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
        ...
        with open('./FBNE_intra_file_upstream/lastfm_3rd_item_valid_file.csv', 'r') as f:
            line = f.readline()
            while line != "" and line != None:
                arr = line.strip().split(',')
                item, high_order_items = int(arr[0]), arr[1:]
    ...
    def get_train_instances(self, epoch):
        ...
        u_1st_item, u_2nd_user, u_3rd_item, u_intra_2nd, u_intra_3rd, u_oracle_user_ebd, u_mask_num_2nd_user, u_mask_num_3rd_item = [], [], [], [], [], [], [], []
        ipos_1st_user, ipos_2nd_item, ipos_3rd_user, ipos_intra_2nd, ipos_intra_3rd, ipos_oracle_item_ebd, ipos_mask_num_2nd_item, ipos_mask_num_3rd_user = [], [], [], [], [], [], [], []
        ineg_1st_user, ineg_2nd_item, ineg_3rd_user, ineg_intra_2nd, ineg_intra_3rd, ineg_oracle_item_ebd, ineg_mask_num_2nd_item, ineg_mask_num_3rd_user = [], [], [], [], [], [], [], []
        ...
        for u in training_dict.keys():
            for i in training_dict[u]:
                ...
                # user neighbor
                per_1st_i, per_2nd_u, per_3rd_i, per_oracle_u_ebd, per_mask_num_2nd_u, per_mask_num_3rd_i = self._get_user_neighbors(
                    u)

                u_1st_item.append(per_1st_i)
                u_2nd_user.append(per_2nd_u)
                u_3rd_item.append(per_3rd_i)
                u_intra_2nd.append(self.train_user_dict_2nd[u])
                u_intra_3rd.append(self.train_user_dict_3rd[u])
                ...
                per_1st_upos, per_2nd_ipos, per_3rd_upos, per_oracle_ipos_ebd, per_mask_num_2nd_ipos, per_mask_num_3rd_upos = self._get_item_neighbors(
                    i)

                ipos_1st_user.append(per_1st_upos)
                ipos_2nd_item.append(per_2nd_ipos)
                ipos_3rd_user.append(per_3rd_upos)
                ipos_intra_2nd.append(self.train_item_dict_2nd[i])
                ipos_intra_3rd.append(self.train_item_dict_3rd[i])
                ...
                per_1st_uneg, per_2nd_ineg, per_3rd_uneg, per_oracle_ineg_ebd, per_mask_num_2nd_ineg, per_mask_num_3rd_uneg = self._get_item_neighbors(
                    j)

                ineg_1st_user.append(per_1st_uneg)
                ineg_2nd_item.append(per_2nd_ineg)
                ineg_3rd_user.append(per_3rd_uneg)
                ineg_intra_2nd.append(self.train_item_dict_2nd[j])
                ineg_intra_3rd.append(self.train_item_dict_3rd[j])
        ...
        u_intra_2nd = np.array(u_intra_2nd)
        u_intra_3rd = np.array(u_intra_3rd)
        ...
        ipos_intra_2nd = np.array(ipos_intra_2nd)
        ipos_intra_3rd = np.array(ipos_intra_3rd)
        ...
        ineg_intra_2nd = np.array(ineg_intra_2nd)
        ineg_intra_3rd = np.array(ineg_intra_3rd)
        ...
        return user_input, u_1st_item, u_2nd_user, u_3rd_item, u_intra_2nd, u_intra_3rd, u_oracle_user_ebd, u_mask_num_2nd_user, u_mask_num_3rd_item, pos_item_input, ipos_1st_user, ipos_2nd_item, ipos_3rd_user, ipos_intra_2nd, ipos_intra_3rd, ipos_oracle_item_ebd, ipos_mask_num_2nd_item, ipos_mask_num_3rd_user, neg_item_input, ineg_1st_user, ineg_2nd_item, ineg_3rd_user, ineg_intra_2nd, ineg_intra_3rd, ineg_oracle_item_ebd, ineg_mask_num_2nd_item, ineg_mask_num_3rd_user
    ...
    def get_test_users(self):
        ...
        u_1st_item, u_2nd_user, u_3rd_item, u_intra_2nd, u_intra_3rd, u_oracle_user_ebd, u_mask_num_2nd_user, u_mask_num_3rd_item = [], [], [], [], [], [], [], []
        i_1st_user, i_2nd_item, i_3rd_user, i_intra_2nd, i_intra_3rd, i_oracle_item_ebd, i_mask_num_2nd_item, i_mask_num_3rd_user = [], [], [], [], [], [], [], []
        ...
        for test_user in test_user_set:
            ...
            u_2nd_user.append(per_2nd_u)
            u_3rd_item.append(per_3rd_i)
            u_intra_2nd.append(self.train_user_dict_2nd[test_user])
            u_intra_3rd.append(self.train_user_dict_3rd[test_user])
            u_oracle_user_ebd.append(per_oracle_u_ebd)
            u_mask_num_2nd_user.append(per_mask_num_2nd_u)
            u_mask_num_3rd_item.append(per_mask_num_3rd_i)
        ...
        for item_index in range(setting.num_items):
            ...
            # item neighbor
            per_1st_u, per_2nd_i, per_3rd_u, per_oracle_i_ebd, per_mask_num_2nd_i, per_mask_num_3rd_u = self._get_item_neighbors(
                item_index)

            i_1st_user.append(per_1st_u)
            i_2nd_item.append(per_2nd_i)
            i_3rd_user.append(per_3rd_u)
            i_intra_2nd.append(self.train_item_dict_2nd[item_index])
            i_intra_3rd.append(self.train_item_dict_3rd[item_index])
        ...
        i_intra_2nd = np.array(i_intra_2nd)
        i_intra_3rd = np.array(i_intra_3rd)
        ...
        return test_user_input, test_user_pos_item, test_user_ground_truth, u_1st_item, u_2nd_user, u_3rd_item, u_intra_2nd, u_intra_3rd, u_oracle_user_ebd, u_mask_num_2nd_user, u_mask_num_3rd_item, i_1st_user, i_2nd_item, i_3rd_user, i_intra_2nd, i_intra_3rd, i_oracle_item_ebd, i_mask_num_2nd_item, i_mask_num_3rd_user


    def batch_gen_3rd_user_task(self, batches, i):
        return [(batches[r])[i] for r in range(9)]

    def batch_gen_3rd_item_task(self, batches, i):
        return [(batches[r])[i] for r in range(9)]

    def batch_gen_mix_user_task(self, batches, i):
        temp = min((i + 1) * setting.batch_size_user, setting.num_users)

        return [(batches[r])[i * setting.batch_size_user: temp, :] for r in range(9)]

    def batch_gen_mix_item_task(self, batches, i):
        temp = min((i + 1) * setting.batch_size_item, setting.num_items)

        return [(batches[r])[i * setting.batch_size_item: temp, :] for r in range(9)]
    ...
    def generate_all_users_neighbors(self):
        ...
        for user_index in range(self.num_users):
            ...
            u_2nd_user.append(per_2nd_u)
            u_3rd_item.append(per_3rd_i)
            u_intra_2nd.append(self.train_user_dict_2nd[user_index])
            u_intra_3rd.append(self.train_user_dict_3rd[user_index])
            u_oracle_user_ebd.append(per_oracle_u_ebd)
            u_mask_num_2nd_user.append(per_mask_num_2nd_u)
            u_mask_num_3rd_item.append(per_mask_num_3rd_i)
        ...
        return user_input, u_1st_item, u_2nd_user, u_3rd_item, u_intra_2nd, u_intra_3rd, u_oracle_user_ebd, u_mask_num_2nd_user, u_mask_num_3rd_item

    def generate_all_items_neighbors(self):
        item_input, i_1st_user, i_2nd_item, i_3rd_user, i_intra_2nd, i_intra_3rd, i_oracle_item_ebd, i_mask_num_2nd_item, i_mask_num_3rd_user = [], [], [], [], [], [], [], [], []
        ...
        return item_input, i_1st_user, i_2nd_item, i_3rd_user, i_intra_2nd, i_intra_3rd, i_oracle_item_ebd, i_mask_num_2nd_item, i_mask_num_3rd_user
    ...
    def get_test_users_revised(self):
        ...
        for test_user in test_user_set:
            ...
            u_intra_2nd.append(self.test_user_dict_2nd[test_user])
            u_intra_3rd.append(self.test_user_dict_3rd[test_user])
        ...
        for item_index in range(setting.num_items):
            ...
            i_intra_2nd.append(self.test_item_dict_2nd[item_index])
            i_intra_3rd.append(self.test_item_dict_3rd[item_index])
