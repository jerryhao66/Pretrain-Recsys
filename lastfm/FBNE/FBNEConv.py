...
class GeneralGNN(object):
    ...
    def _create_placeholder(self):
        ...
        # fbne
        self.inter_support_2nd_item_pos = tf.placeholder(tf.int32, shape=[None, None])
        self.inter_support_2nd_item_neg = tf.placeholder(tf.int32, shape=[None, None])
        self.inter_support_3rd_item_pos = tf.placeholder(tf.int32, shape=[None, None])
        self.inter_support_3rd_item_neg = tf.placeholder(tf.int32, shape=[None, None])

        self.inter_support_2nd_user = tf.placeholder(tf.int32, shape=[None, None])
        self.inter_support_3rd_user = tf.placeholder(tf.int32, shape=[None, None])
    ...
    def _create_bpr_loss(self, users, pos_items, neg_items, w):
        ...
        # test set use
        self.batch_predicts = tf.sigmoid(tf.matmul(tf.multiply(users, pos_items), w))
        ...
        if name == 'FBNE':
            ...
            with tf.name_scope('FBNE_2nd_i'):
                with tf.variable_scope('FBNE_2nd_i', reuse=tf.AUTO_REUSE):
                    # perform GraphCSC + implicit item-item self attention
                    ...
                    support_inter_ori_ebd_2nd = tf.nn.embedding_lookup(self.context_embedding_i,
                                                                       self.inter_support_2nd_item_pos)
            ...
            with tf.name_scope('FBNE_3rd_i'):
                with tf.variable_scope('FBNE_3rd_i', reuse=tf.AUTO_REUSE):
                    ...
                    support_inter_ori_ebd_3rd = tf.nn.embedding_lookup(self.context_embedding_i,
                                                                       self.inter_support_3rd_item_pos)
    ...
    def produce_reward_user_task(self, _target_user, b_k_shot_item, selected_input_2nd, b_oracle_user_ebd, b_intra_2nd_user):
        ...
        first_and_second_order_reward2, batch_predict_ebd, batch_target_ebd = self.sess.run(
            ...
            feed_dict={self.target_user: b_oracle_user_ebd,
                       ...
                       self.inter_support_2nd_user: b_intra_2nd_user,
    ...
    def produce_reward_3rd_user_task(self, _target_user, b_k_shot_item, selected_input_2nd, selected_input_3rd,
        ...
        feed_dict = {self.target_user: b_oracle_user_ebd, self.support_item_1st_: b_k_shot_item,
                     ...
                     self.training_phrase_item_task: False,
                     self.inter_support_3rd_user: b_intra_3rd_user,
    ...
    def produce_original_reward_user_task(self, train_batches, num_batch_train, ClassData):
        ...
        for batch_index in range(num_batch_train):
            b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item, b_intra_2nd_user, b_intra_3rd_user = ClassData.batch_gen_3rd_user_task(
    ...
    def generate_batch_state_ebd_user_task(self, train_batches, ClassData):
        all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_intra_2nd_ori_ebd, all_intra_3rd_ori_ebd,  all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3 = [], [], [], [], [], [], [], [], [], [], []
        ...
        for index in train_batch_index:
            ...
            first_neigh_ori_ebd = self.original_item_ebd[np.array(b_k_shot_item)]
            second_neigh_ori_ebd = self.original_user_ebd[np.array(b_second_order_users)]
            third_neigh_ori_ebd = self.original_item_ebd[np.array(b_third_order_items)]
            ...
            all_intra_2nd_ori_ebd.append(b_intra_2nd_user)
            all_intra_3rd_ori_ebd.append(b_intra_3rd_user)
        ...
        return all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_intra_2nd_ori_ebd, all_intra_3rd_ori_ebd, all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3
    ...
    def produce_reward_item_task(self, _target_item, b_k_shot_user, selected_input, b_oracle_item_ebd, b_intra_2nd_item):
        ...
        first_and_second_order_reward2, batch_predict_ebd, batch_target_ebd = self.sess.run(
            ...
            feed_dict={self.target_item: b_oracle_item_ebd,
                       ...
                       self.inter_support_2nd_item_pos: b_intra_2nd_item,
                       self.training_phrase_item_task: False})
        first_and_second_order_reward2 = np.reshape(first_and_second_order_reward2, (-1))
        batch_pearson = Pearson_correlation(batch_predict_ebd, batch_target_ebd)
    ...
    def produce_reward_3rd_item_task(self, _target_item, b_k_shot_user, selected_input_2nd, selected_input_3rd,
        ...
        feed_dict = {self.target_item: b_oracle_item_ebd, self.support_user_1st_pos_: b_k_shot_user,
                     ...
                     self.training_phrase_item_task: False,
                     self.inter_support_3rd_item_pos: b_intra_3rd_item,
    ...
    def produce_original_reward_item_task(self, train_batches, num_batch_train, ClassData):
        original_reward = 0.0
        for batch_index in range(num_batch_train):
            b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user, b_intra_2nd_item, b_intra_3rd_item = ClassData.batch_gen_3rd_item_task(
                train_batches, batch_index)
    ...
    def generate_batch_state_ebd_item_task(self, train_batches, ClassData):
        ...
        for index in train_batch_index:
            ...
            first_neigh_ori_ebd = self.original_user_ebd[np.array(b_k_shot_user)]
            second_neigh_ori_ebd = self.original_item_ebd[np.array(b_second_order_items)]
            third_neigh_ori_ebd = self.original_user_ebd[np.array(b_third_order_users)]
            ...
            all_second_neigh_ori_ebd.append(second_neigh_ori_ebd)
            all_third_neigh_ori_ebd.append(third_neigh_ori_ebd)
            all_intra_2nd_ori_ebd.append(b_intra_2nd_item)
            all_intra_3rd_ori_ebd.append(b_intra_3rd_item)
    ...
    def evaluate_item_task(self):
        ...
        evaluate_loss, evaluate_pearson = 0.0, 0.0
        for index in tqdm.tqdm(valid_batch_index):
            b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user, b_intra_2nd_item, b_intra_3rd_item = data_valid.batch_gen_3rd_item_task(
                valid_batches, index)
    ...
    def generate_batch_state_ebd_item_task_downstream(self, train_batches, ClassData):
        ...
        train_batch_index = range(num_batch_train)
        for index in train_batch_index:
            b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_intra_2nd_item, b_intra_3rd_item, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user = ClassData.batch_gen_mix_item_task(
                train_batches, index)
            ...
            intra_2nd_ori_ebd = self.original_item_ebd[b_intra_2nd_item]
            intra_3rd_ori_ebd = self.original_item_ebd[b_intra_3rd_item]
            ...
            all_second_neigh_ori_ebd.append(second_neigh_ori_ebd)
            all_third_neigh_ori_ebd.append(third_neigh_ori_ebd)
            all_intra_2nd_ori_ebd.append(intra_2nd_ori_ebd)
            all_intra_3rd_ori_ebd.append(intra_3rd_ori_ebd)
    ...
    def generate_batch_state_ebd_user_task_downstream(self, train_batches, ClassData):
        ...
        train_batch_index = range(num_batch_train)
        for index in train_batch_index:
            b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_intra_2nd_user, b_intra_3rd_user, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item = ClassData.batch_gen_mix_user_task(
                train_batches, index)
            ...
            intra_2nd_ori_ebd = self.original_user_ebd[b_intra_2nd_user]
            intra_3rd_ori_ebd = self.original_user_ebd[b_intra_3rd_user]
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        model = GeneralGNN('FBNE', sess)
        sess.run(tf.global_variables_initializer())
        logging.info("initialized")
        print('training user task...')
        training_user_task_fbne(model, sess)
        training_2nd_user_task_fbne(model, sess)
        training_3rd_user_task_fbne(model, sess)
        train_user_rl_task('FBNE')
        revise_rl_user_task('FBNE')
        print('training item task...')
        training_item_task_fbne(model, sess)
        training_2nd_item_task_fbne(model, sess)
        training_3rd_item_task_fbne(model, sess)
        train_item_rl_task('FBNE')
        revise_rl_item_task('FBNE')
