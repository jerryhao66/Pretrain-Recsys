...
import tqdm
import gendata_fastgcn_new as data
...
class GeneralGNN(object):
    def __init__(self, name, sess):
        ...
        self.batch_size_recommender = setting.batch_size_recommender
        ...
        with tf.device('/gpu:3'):
            ...
            self.learning_rate_agent = tf.train.exponential_decay(self.lr, self.global_step, 10000, 0.95, staircase=True)
            ...
            self.second_order_input_state, _ = self.create_agent_network("Activate/second", self.second_order_state_size)
            self.second_order_network_params = tf.trainable_variables()[self.num_other_variables:]

            # Agent network(delayed updating)
            self.target_second_order_input_state, self.target_second_order_prob = self.create_agent_network("Target/second",
            ...
            self.second_order_pi = self.second_order_action_holder * self.target_second_order_prob + (
                    1 - self.second_order_action_holder) * (1 - self.target_second_order_prob)
            ...
            self.third_order_network_params = tf.trainable_variables()[self.num_other_variables + len(self.second_order_network_params) + len(self.target_second_order_network_params):]

            # Agent network(delayed updating)
            self.target_third_order_input_state, self.target_third_order_prob = self.create_agent_network("Target/third", self.third_order_state_size)
            self.target_third_order_network_params = tf.trainable_variables()[self.num_other_variables + len(self.second_order_network_params) + len(self.target_second_order_network_params) + len(self.third_order_network_params):]
            ...
            self.third_order_pi = self.third_order_action_holder * self.target_third_order_prob + (
                    1 - self.third_order_action_holder) * (1 - self.target_third_order_prob)
            ...
            self.concat_user_item_ebd = np.load(setting.fastgcn_pre_train_concat_ebd)
    ...
    def _2nd_pairwise_loss(self):
        # variable
        self.second_w_pos = self.glorot(shape=[self.embedding_size, 1], name='second_w_pos')

        self.second_mf_loss, self.second_emb_loss, self.second_reg_loss = self._create_bpr_loss(
            self.predict_u_2nd,
            self.predict_i_2nd_pos,
            self.predict_i_2nd_neg, self.second_w_pos)

        self.second_loss = self.second_mf_loss + self.second_emb_loss
        self.second_opt = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.second_loss)

    def _3rd_pairwise_loss(self):
        # variable
        self.third_w_pos = self.glorot(shape=[self.embedding_size, 1], name='third_w_pos')

        self.third_mf_loss, self.third_emb_loss, self.third_reg_loss = self._create_bpr_loss(
            self.predict_u_3rd,
            self.predict_i_3rd_pos,
            self.predict_i_3rd_neg, self.third_w_pos)

        self.third_loss = self.third_mf_loss + self.third_emb_loss
        self.third_opt = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.third_loss)

    def _create_bpr_loss(self, users, pos_items, neg_items, w):

        pos_scores = tf.sigmoid(tf.matmul(tf.multiply(users, pos_items), w))
        neg_scores = tf.sigmoid(tf.matmul(tf.multiply(users, neg_items), w))

        # test set use
        self.batch_predicts = pos_scores

        regularizer = tf.nn.l2_loss(users) + tf.nn.l2_loss(pos_items) + tf.nn.l2_loss(neg_items)
        regularizer = regularizer / self.batch_size_recommender
        mf_loss = tf.reduce_mean(tf.maximum(0.0, 1 - pos_scores + neg_scores))
    ...
    def _create_aggregator_network_item_task(self, scope):
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            ...
            self.context_embedding_concat_item_task = tf.concat([self.c1, self.c2], 0, name='embedding_item')

        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            self.support_ebd = tf.nn.embedding_lookup(self.context_embedding_concat_item_task, self.support_user)  # [b, n, e]
    ...
    def _create_aggregator_network_user_task(self, scope):
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            ...
            self.c1 = tf.Variable(tf.constant(pre_train_item_ebd, shape=[self.num_items, self.embedding_size]),
                                  trainable=False)
            self.c2 = tf.constant(0.0, tf.float32, [1, self.embedding_size], name='c2')
            self.context_embedding_concat_user_task = tf.concat([self.c1, self.c2], 0, name='embedding_user')

        with tf.name_scope(scope):
            self.support_ebd = tf.nn.embedding_lookup(self.context_embedding_concat_user_task, self.support_item)  # [b, n, e]
    ...
    def _2nd_user_task(self, name, support_user_2nd, support_item_1st):
        if name == 'FastGCN':
            print('use FastGCN')
            with tf.name_scope('FastGCN_2nd_u'):
                with tf.variable_scope('FastGCN_2nd_u', reuse=tf.AUTO_REUSE):
                    ...
                    support_ori_ebd_2nd = tf.nn.embedding_lookup(self.context_embedding_concat_user_task,
                    ...
                    support_ori_ebd_1st = tf.nn.embedding_lookup(self.context_embedding_concat_user_task,
    ...
    def _2nd_item_task(self, name, support_item_2nd, support_user_1st):
        ...
        if name == 'FastGCN':
            print('use FastGCN')
            with tf.name_scope('FastGCN_2nd_i'):
                with tf.variable_scope('FastGCN_2nd_i', reuse=tf.AUTO_REUSE):
                    ...
                    support_ori_ebd_2nd = tf.nn.embedding_lookup(self.context_embedding_concat_item_task, support_item_2nd)
                    support_encode_2nd = tf.reduce_mean(
                        self._encode(support_ori_ebd_2nd, training=self.training_phrase_item_task), 1)
                    ori_2nd_ebd = tf.reduce_mean(support_ori_ebd_2nd, 1)  # [b, e]


                    support_ori_ebd_1st = tf.nn.embedding_lookup(self.context_embedding_concat_item_task, support_user_1st)
    ...
    def _3rd_item_task(self, name, support_user_3rd, support_item_2nd_, support_user_1st_):
        if name == 'FastGCN':
            print('use FastGCN')
            with tf.name_scope('FastGCN_3rd_i'):
                with tf.variable_scope('FastGCN_3rd_i', reuse=tf.AUTO_REUSE):
                    ...
                    support_ori_ebd_3rd = tf.nn.embedding_lookup(self.context_embedding_concat_item_task,
                    ...
                    support_ori_ebd_2nd = tf.nn.embedding_lookup(self.context_embedding_concat_item_task,
                                                                 support_item_2nd_)  # [b, n, e]
                    support_encode_2nd = tf.reduce_mean(
                        self._encode(support_ori_ebd_2nd, training=self.training_phrase_item_task), 1)


                    support_ori_ebd_1st = tf.nn.embedding_lookup(self.context_embedding_concat_item_task,
                                                                 support_user_1st_)
    ...
    def _3rd_user_task(self, name, support_item_3rd, support_user_2nd_, support_item_1st_):
        ...
        if name == 'FastGCN':
            print('use FastGCN')
            with tf.name_scope('FastGCN_3rd_u'):
                with tf.variable_scope('FastGCN_3rd_u', reuse=tf.AUTO_REUSE):
                    ...
                    support_ori_ebd_3rd = tf.nn.embedding_lookup(self.context_embedding_concat_user_task,
    ...
    def produce_original_reward_user_task(self):
        original_reward = 0.0
        ...
        data = Dataset('./Mooc_upstream/user_task/user_task_train_oracle_rating.csv')
        target_user, k_shot_item, second_order_uesrs, third_order_items, oracle_user_ebd, mask_num_second_order_user, mask_num_third_order_item = data.get_positive_instances_user_task(
            data, all_dict)

        num_batch_train = int(len(target_user) // setting.fastgcn_batch_size)
        train_batch_index = range(num_batch_train)
        for batch_index in train_batch_index:
    ...
    def produce_original_reward_3rd_user_task(self):
    ...
    def generate_batch_state_ebd_user_task(self):
        num_users = setting.num_users  
        num_items = setting.num_items  
        all_dict = id_to_useritemid(num_users, num_items)
        all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3 = [], [], [], [], [], [], [], [], []
        ...
        for index in train_batch_index:
            b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item = split_batch_user(target_user, k_shot_item, second_order_uesrs, third_order_items, oracle_user_ebd, mask_num_second_order_user, mask_num_third_order_item, index)



            target_ori_ebd = self.concat_user_item_ebd[b_target_user]
            ...
            first_neigh_ori_ebd = self.concat_user_item_ebd[b_k_shot_item]
            second_neigh_ori_ebd = self.concat_user_item_ebd[b_second_order_users]
            third_neigh_ori_ebd = self.concat_user_item_ebd[b_third_order_items]
    ...
    def evaluate_user_task(self):
        ...
        all_dict = id_to_useritemid(num_users, num_items)
        all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3 = [], [], [], [], [], [], [], [], []
        data = Dataset('../Mooc_upstream/user_task/user_task_valid_oracle_rating.csv')
        target_user, k_shot_item, second_order_uesrs, third_order_items, oracle_user_ebd, mask_num_second_order_user, mask_num_third_order_item = data.get_positive_instances_user_task(
            data, all_dict)


        num_batch_valid = int(len(target_user) // setting.fastgcn_batch_size )
        ...
        for index in tqdm.tqdm(valid_batch_index):
            b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item = split_batch_user(
                target_user, k_shot_item, second_order_uesrs, third_order_items, oracle_user_ebd,
                mask_num_second_order_user, mask_num_third_order_item, index)
    ...
    def produce_original_reward_item_task(self):
        original_reward = 0.0
        num_users = setting.num_users  
        num_items = setting.num_items  
        all_dict = id_to_useritemid(num_users, num_items)
        all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3 = [], [], [], [], [], [], [], [], []
        data = Dataset('./Mooc_upstream/item_task/item_task_train_oracle_rating.csv')
        target_item, k_shot_user, second_order_items, third_order_users, oracle_item_ebd, mask_num_second_order_item, mask_num_third_order_user = data.get_positive_instances_item_task(
            data, all_dict)

        num_batch_train = int(len(target_item) // setting.fastgcn_batch_size)
        train_batch_index = range(num_batch_train)
        for batch_index in train_batch_index:
            b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user = split_batch_item(
                target_item, k_shot_user, second_order_items, third_order_users, oracle_item_ebd,
                mask_num_second_order_item, mask_num_third_order_user, batch_index)
    ...
    def produce_original_reward_3rd_item_task(self):
        original_reward = 0.0
        num_users = setting.num_users  
        num_items = setting.num_items 
        all_dict = id_to_useritemid(num_users, num_items)
    ...
    def generate_batch_state_ebd_item_task(self):
        ...
        for index in train_batch_index:
            b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user = split_batch_item(
                target_item, k_shot_user, second_order_items, third_order_users, oracle_item_ebd,
                mask_num_second_order_item, mask_num_third_order_user, index)


            target_ori_ebd = self.concat_user_item_ebd[b_target_item]
            ...
            first_neigh_ori_ebd = self.concat_user_item_ebd[b_k_shot_user]
            second_neigh_ori_ebd = self.concat_user_item_ebd[b_second_order_items]
            third_neigh_ori_ebd = self.concat_user_item_ebd[b_third_order_users]
    ...
    def evaluate_item_task(self):
        ...
        all_dict = id_to_useritemid(num_users, num_items)
        all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3 = [], [], [], [], [], [], [], [], []
        data = Dataset(setting.oracle_valid_file_item_task)
        target_item, k_shot_user, second_order_items, third_order_users, oracle_item_ebd, mask_num_second_order_item, mask_num_third_order_user = data.get_positive_instances_item_task(
            data, all_dict)

        num_batch_valid = int(len(target_item) // setting.fastgcn_batch_size)
        valid_batch_index = range(num_batch_valid)
    ...
    def generate_batch_state_ebd_item_task_downstream(self, train_batches, ClassData):
        ...
        for index in train_batch_index:
            ...
            first_neigh_ori_ebd = self.concat_user_item_ebd[np.array(b_k_shot_user)]
            second_neigh_ori_ebd = self.concat_user_item_ebd[np.array(b_second_order_items)]
            third_neigh_ori_ebd = self.concat_user_item_ebd[np.array(b_third_order_users)]
    ...
    def generate_batch_state_ebd_user_task_downstream(self, train_batches, ClassData):
        ...
        for index in train_batch_index:
            ...
            target_ori_ebd = self.concat_user_item_ebd[np.array(b_target_user)]
            ...
            first_neigh_ori_ebd = self.concat_user_item_ebd[np.array(b_k_shot_item)]
            second_neigh_ori_ebd = self.concat_user_item_ebd[np.array(b_second_order_users)]
            third_neigh_ori_ebd = self.concat_user_item_ebd[np.array(b_third_order_items)]
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        model = GeneralGNN('FastGCN', sess)
        sess.run(tf.global_variables_initializer())
        logging.info("initialized")
        print('training user task...')
        training_user_task_fastgcnnew(model, sess)
        training_2nd_user_task_fastgcnnew(model, sess)
        training_3rd_user_task_fastgcnnew(model, sess)
        train_user_rl_task('FastGCN')
        revise_rl_user_task('FastGCN')
        print('training item task...')
        training_item_task_fastgcnnew(model, sess)
        training_2nd_item_task_fastgcnnew(model, sess)
        training_3rd_item_task_fastgcnnew(model, sess)
        train_item_rl_task('FastGCN')
        revise_rl_item_task('FastGCN')
