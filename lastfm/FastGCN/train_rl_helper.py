...
from FastGCNConv import *
import torch
import tqdm
os.environ["CUDA_VISIBLE_DEVICES"] = '0,1,2,3'
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'
import os
from gendata_fastgcn_new import *
...
class environment():
    def __init__(self):
        ...
        self.fastgcn_batch_size = setting.fastgcn_batch_size
    ...
    def initilize_state_user_task(self, max_num, state_size, batch_original_reward, original_reward):
        ...
        self.origin_prob = np.zeros((self.fastgcn_batch_size, 1), dtype=np.float32) 
        self.dot_product = np.zeros((self.fastgcn_batch_size, 1), dtype=np.float32)  
        self.element_wise_current_mean1 = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum1 = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean2 = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum2 = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean3 = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum3 = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.vector_current_sum1 = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.vector_current_mean1 = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.num_selected = np.zeros(self.fastgcn_batch_size, dtype=np.int)
        self.action_matrix = np.zeros((self.fastgcn_batch_size, self.max_num), dtype=np.int)
        self.state_matrix = np.zeros((self.fastgcn_batch_size, self.max_num, state_size), dtype=np.float32)
        self.selected_input = np.full((self.fastgcn_batch_size, self.max_num), self.padding_number_user_task)

    def initilize_state_user_task_3rd(self, max_num_3rd, state_size_3rd):
        self.max_num_3rd = max_num_3rd
        self.origin_prob_3rd = np.zeros((self.fastgcn_batch_size, 1), dtype=np.float32)  
        self.dot_product_3rd = np.zeros((self.fastgcn_batch_size, 1), dtype=np.float32)  
        self.element_wise_current_mean1_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum1_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean2_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum2_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean3_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum3_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean4_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum4_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_mean5_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.element_wise_current_sum5_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.vector_current_sum1_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.vector_current_mean1_3rd = np.zeros((self.fastgcn_batch_size, self.embedding_size), dtype=np.float32)
        self.num_selected_3rd = np.zeros(self.fastgcn_batch_size, dtype=np.int)
        self.action_matrix_3rd = np.zeros((self.fastgcn_batch_size, self.max_num_3rd), dtype=np.int)
        self.state_matrix_3rd = np.zeros((self.fastgcn_batch_size, self.max_num_3rd, state_size_3rd), dtype=np.float32)
        self.selected_input_3rd = np.full((self.fastgcn_batch_size, self.max_num_3rd), self.padding_number_user_task)
    ...
    def initilize_state_item_task(self, max_num, state_size, batch_original_reward, original_reward):
        ...
        self.action_matrix = np.zeros((self.fastgcn_batch_size, self.max_num), dtype=np.int)
        self.state_matrix = np.zeros((self.fastgcn_batch_size, self.max_num, state_size), dtype=np.float32)
        self.selected_input = np.full((self.fastgcn_batch_size, self.max_num), self.padding_number_item_task)
    ...
    def initilize_state_item_task_3rd(self, max_num_3rd, state_size_3rd):
        ...
        self.action_matrix_3rd = np.zeros((self.fastgcn_batch_size, self.max_num_3rd), dtype=np.int)
        self.state_matrix_3rd = np.zeros((self.fastgcn_batch_size, self.max_num_3rd, state_size_3rd), dtype=np.float32)
        self.selected_input_3rd = np.full((self.fastgcn_batch_size, self.max_num_3rd), self.padding_number_item_task)
    ...
    def get_selected_second_order_neighbors_item_task(self, b_second_order_item):
        ...
        row_deleted_flag = np.ones((self.fastgcn_batch_size), dtype=np.int)

        selected_input = np.zeros((self.fastgcn_batch_size, self.max_num), dtype=np.int)
        for batch_index in range(self.fastgcn_batch_size):
            selected = []
    ...
    def get_selected_third_order_neighbors_item_task(self, b_third_order_users, row_deleted_flag_2rd):
        row_deleted_index = []
        selected_input = np.zeros((self.fastgcn_batch_size, self.max_num_3rd), dtype=np.int)
        for batch_index in range(self.fastgcn_batch_size):
            selected = []
            ...
            self.num_selected_3rd = self.fastgcn_batch_size * self.max_num_3rd - np.sum(
...
def generate_batch_user_task(batch_index, model, sess, train_data, is_training, num_instances):
    ...
    for index in batch_index:
        _, support_item = data.batch_gen_task(train_data, index, setting.fastgcn_batch_size, num_instances)
        ...
        start_index = index * setting.fastgcn_batch_size
        end_index = min(start_index + setting.fastgcn_batch_size, num_instances)
...
def generate_batch_item_task(batch_index, model, sess, train_data, is_training, num_instances):
    ...
    for index in batch_index:
        all_item_id, support_user = data.batch_gen_task(train_data, index, setting.fastgcn_batch_size, num_instances)
...
def train_user_rl_task(name):
    ...
    fastgcn_batch_size = setting.fastgcn_batch_size
    ...
    with g_rl.as_default():
        with sess2.as_default():
            ...
            ClassData = Dataset('./Mooc_upstream/user_task/user_task_valid_oracle_rating.csv')
            num_users = setting.num_users  
            num_items = setting.num_items  
            all_dict = id_to_useritemid(num_users, num_items)

            for epoch in range(40): # 80
                train_batches = ClassData.get_positive_instances_user_task(ClassData, all_dict)
                num_batch_train = int(len(train_batches[0]) // setting.fastgcn_batch_size)
                print('num_batch_train is %d' % num_batch_train)
                ...
                target_user, k_shot_item, second_order_uesrs, third_order_items, oracle_user_ebd, mask_num_second_order_user, mask_num_third_order_item = train_batches
                for rl_epoch in range(1):
                    ...
                    for batch_index in tqdm.tqdm(range(num_batch_train)):
                        ...
                        b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item = split_batch_user(
                            target_user, k_shot_item, second_order_uesrs, third_order_items, oracle_user_ebd,
                            mask_num_second_order_user, mask_num_third_order_item, batch_index)
                        ...
                        original_reward_2rd = model.produce_original_reward_user_task()
                        original_reward_3rd = model.produce_original_reward_3rd_user_task()
                        ...
                        sampled_rewards = np.zeros((sample_times, fastgcn_batch_size), dtype=np.float32)
                        sampled_states = np.zeros((sample_times, fastgcn_batch_size, max_num, state_size),
                                                  dtype=np.float32)
                        sampled_actions = np.zeros((sample_times, fastgcn_batch_size, max_num), dtype=np.float32)

                        sampled_rewards_3rd = np.zeros((sample_times, fastgcn_batch_size), dtype=np.float32)
                        sampled_states_3rd = np.zeros((sample_times, fastgcn_batch_size, max_num_3rd, state_size_3rd),
                                                      dtype=np.float32)
                        sampled_actions_3rd = np.zeros((sample_times, fastgcn_batch_size, max_num_3rd),
                                                       dtype=np.float32)

                        avg_reward = np.zeros((fastgcn_batch_size), dtype=np.float32)
...
def revise_rl_user_task(name):
    ...
    with g_rl_revise.as_default():
        with sess3.as_default():
            ...
            ClassData = Dataset(setting.oracle_valid_file_user_task)
            num_users = setting.num_users  
            num_items = setting.num_items  
            all_dict = id_to_useritemid(num_users, num_items)
            valid_batches = ClassData.get_positive_instances_user_task(ClassData, all_dict)
            num_batch_valid = int(len(valid_batches[0]) // setting.fastgcn_batch_size)
            valid_batch_index = range(num_batch_valid)
            print('num_batch_train is %d' % num_batch_valid)

            all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3 = model.generate_batch_state_ebd_user_task()
            target_user, k_shot_item, second_order_uesrs, third_order_items, oracle_user_ebd, mask_num_second_order_user, mask_num_third_order_item = valid_batches
            for batch_index in tqdm.tqdm(valid_batch_index):
                b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item = split_batch_user(target_user, k_shot_item, second_order_uesrs, third_order_items, oracle_user_ebd, mask_num_second_order_user, mask_num_third_order_item, batch_index)
...
def train_item_rl_task(name):
    ...
    with g_rl.as_default():
        with sess2.as_default():
            ...
            ClassData = Dataset(setting.oracle_training_file_item_task)
            num_users = setting.num_users  
            num_items = setting.num_items  
            all_dict = id_to_useritemid(num_users, num_items)

            for epoch in range(1): 
                train_batches = ClassData.get_positive_instances_item_task(ClassData, all_dict)
                num_batch_train = int(len(train_batches[0]) // setting.fastgcn_batch_size)
                ...
                env.assign_batch_data(train_batches)
                target_item, k_shot_user, second_order_items, third_order_users, oracle_item_ebd, mask_num_second_order_item, mask_num_third_order_user = train_batches
                for rl_epoch in range(1):
                    train_begin = time()
                    avg_total_reward = 0
                    avg_update_total_reward = 0
                    ...
                    for batch_index in tqdm.tqdm(range(num_batch_train)):
                        ...
                        b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user = split_batch_item(target_item, k_shot_user, second_order_items, third_order_users, oracle_item_ebd, mask_num_second_order_item, mask_num_third_order_user, batch_index)
                        ...
                        b_target_ori_ebd, b_target_aggre_ebd, b_target_second_aggre_ebd, b_first_neigh_ori_ebd, b_second_neigh_ori_ebd, batch_third_neigh_ori_ebd, b_prob1, b_prob2, b_prob3 = \
                            ...
                            all_second_neigh_ori_ebd[
                                ...
                                batch_index],  all_first_and_second_order_reward2[
                                batch_index], all_former_third_order_reward3[batch_index]
                        original_reward_2rd = model.produce_original_reward_item_task()
                        original_reward_3rd = model.produce_original_reward_3rd_item_task()
                        ...
                        sampled_rewards = np.zeros((sample_times, fastgcn_batch_size), dtype=np.float32)
                        sampled_states = np.zeros((sample_times, fastgcn_batch_size, max_num, state_size), dtype=np.float32)
                        sampled_actions = np.zeros((sample_times, fastgcn_batch_size, max_num), dtype=np.float32)

                        sampled_rewards_3rd = np.zeros((sample_times, fastgcn_batch_size), dtype=np.float32)
                        sampled_states_3rd = np.zeros((sample_times, fastgcn_batch_size, max_num_3rd, state_size_3rd),
                                                      dtype=np.float32)
                        sampled_actions_3rd = np.zeros((sample_times, fastgcn_batch_size, max_num_3rd), dtype=np.float32)
...
def revise_rl_item_task(name):
    ...
    with g_rl_revise.as_default():
        with sess3.as_default():
            ...
            ClassData = Dataset(setting.oracle_valid_file_item_task)
            num_users = setting.num_users  
            num_items = setting.num_items  
            all_dict = id_to_useritemid(num_users, num_items)
            valid_batches = ClassData.get_positive_instances_item_task(ClassData, all_dict)
            num_batch_valid = int(len(valid_batches[0]) // setting.fastgcn_batch_size)
            valid_batch_index = range(num_batch_valid)
            print('num_batch_train is %d' % num_batch_valid)
            ...
            target_item, k_shot_user, second_order_items, third_order_users, oracle_item_ebd, mask_num_second_order_item, mask_num_third_order_user = valid_batches
            ...
            for batch_index in tqdm.tqdm(valid_batch_index):
                ...
                all_first_neigh_ori_ebd[
                    batch_index], \
                all_second_neigh_ori_ebd[
                    batch_index], all_third_neigh_ori_ebd[batch_index], all_first_order_reward1[batch_index], all_first_and_second_order_reward2[batch_index], all_former_third_order_reward3[batch_index]
